...
def escuchar_confirmacion() -> str:
    ...
    try:
        with sr.Microphone() as source:
            print("🎤 (si / no)...")
            recognizer.adjust_for_ambient_noise(source, duration=1)  # más tiempo calibrando
            recognizer.energy_threshold = 200  # más sensible a sonidos bajos
            recognizer.dynamic_energy_threshold = False  # umbral fijo, más estable
            audio = recognizer.listen(source, timeout=10, phrase_time_limit=6)

        texto = recognizer.recognize_google(audio, language="es-ES").lower()
        print(f"(Confirmación detectada: {texto})")

        if "sí" in texto or "si" in texto or "correcto" in texto or "exacto" in texto or "afirmativo" in texto or "vale" in texto or "claro" in texto:
            return "si"
        if "no" in texto or "incorrecto" in texto or "mal" in texto or "nada" in texto:
            return "no"
